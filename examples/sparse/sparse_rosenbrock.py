...
class Rosenbrock(ParOpt.Problem):
    ...
    def evalObjCon(self, x):
        ...
        # Evaluate the objective and dense constraints
        fail = 0
        con = np.zeros(1)
    ...
    def evalObjConGradient(self, x, g, A):
        ...
        # Evaluate the dense constraint gradient, if any
        return fail
    ...
    def addSparseJacobian(self, alpha, x, px, con):
        ...
        con[0] += alpha * (px[0] + px[1])
        return

    def addSparseJacobianTranspose(self, alpha, x, pz, out):
        """Compute the transpose Jacobian-vector product out += alpha*J^{T}*pz"""
        out[0] += alpha * pz[0]
        out[1] += alpha * pz[0]
        return

    def addSparseInnerProduct(self, alpha, x, c, A):
        """Add the results from the product alpha * J(x)*C*J(x)^{T} to A"""
        A[0] += alpha * (c[0] + c[1])
        return
...
def plot_it_all(problem):
    ...
    # Create the plot
    fig = plt.figure(facecolor="w", figsize=(6.0, 6.0))
    ...
    for k in range(len(colours)):
        ...
        plt.plot(sd[0, :], sd[1, :], colours[k], label="IP nit:%d" % (sd.shape[1]))

...
class TrussAnalysis(ParOpt.Problem):
    ...
    def plotTruss(self, x, tol=None, filename="opt_truss.pdf"):
        ...
        for bar in self.conn:
            ...
            if A[index] >= tol:
                plt.plot(
                    [self.xpos[2 * n1], self.xpos[2 * n2]],
                    [self.xpos[2 * n1 + 1], self.xpos[2 * n2 + 1]],
                    "-ko",
    ...
    def writeOutputFiles(self, A, show=False):
        ...
        if not hasattr(self, "fig"):
            ...
            for bar in self.conn:
                ...
                (line,) = self.ax.plot(xv, yv, "-ko", linewidth=A[index])
            ...
            for bar in self.conn:
                plt.setp(self.lines[index], linewidth=5 * (A[index] / max(A)))
                index += 1

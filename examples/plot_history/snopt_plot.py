...
def snopt_plot(filename, savefig):
    ...
    # metadata line text
    metadata_line = (
        " Major Minors     Step   nCon Feasible  "
    ...
    for line_index in range(datastart_index, dataend_index):
        if lines[line_index] == "\n":
            ...
            try:
                ...
                if not savefig:
                    warnings.warn(
                        "Missing major iteration number found "
                        "in line {:d}".format(line_index + 1),
                        RuntimeWarning,
                    )
            try:
                ...
                if not savefig:
                    warnings.warn(
                        "Missing entry 'Step' found in major "
                        "iteration {:d}".format(int(major[-1])),
                        RuntimeWarning,
                    )
            try:
                ...
                if not savefig:
                    warnings.warn(
                        "Missing entry 'nCon' found in major "
                        "iteration {:d}".format(int(major[-1])),
                        RuntimeWarning,
                    )
            try:
                ...
                if not savefig:
                    warnings.warn(
                        "Missing entry 'Feasible' found in major "
                        "iteration {:d}".format(int(major[-1])),
                        RuntimeWarning,
                    )
            try:
                ...
                if not savefig:
                    warnings.warn(
                        "Missing entry 'Optimal' found in major "
                        "iteration {:d}".format(int(major[-1])),
                        RuntimeWarning,
                    )
            try:
                ...
                if not savefig:
                    warnings.warn(
                        "Missing entry 'MeritFun' found in major "
                        "iteration {:d}".format(int(major[-1])),
                        RuntimeWarning,
                    )
            try:
                ...
                if not savefig:
                    warnings.warn(
                        "Missing entry 'nS' found in major "
                        "iteration {:d}".format(int(major[-1])),
                        RuntimeWarning,
                    )
            try:
                ...
                if not savefig:
                    warnings.warn(
                        "Missing entry 'Penalty' found in major "
                        "iteration {:d}".format(int(major[-1])),
                        RuntimeWarning,
                    )
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "filename", metavar="SNOPT.out", type=str, help="SNOPT summary output file name"
    )

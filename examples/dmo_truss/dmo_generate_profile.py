...
if tikz is None:
    ...
    print perform
    ymin = min(0, np.min(perform.flatten()))
    ymax = max(1, min(20, np.max(perform.flatten())))
    ...
    s += tikz.get_2d_axes(xmin, xmax, ymin, ymax,
                          ...
                          ylabel=variables[imerit]) # '$c_{p}(\mathbf{x}^{*})$/best $c_{p}(\mathbf{x}^{*})$')
    ...
    for k in xrange(len(heuristics)):
        ...
        # Add a label to the legend
        length = 0.1
        s += tikz.get_legend_entry(20, 2.5 - 0.25*k, length,
                                   xscale=xscale, yscale=yscale,
                                   color=colors[k], line_dim='ultra thick',

...
def setup_ground_struct(N, M, L=2.5, E=70e9, x_lb=0.0):
    ...
    truss = TrussAnalysis(
        conn, xpos, loads, bcs, E, rho, Avals, m_fixed, x_lb=x_lb, epsilon=1e-6
    )

    # Set the options
    truss.setInequalityOptions(
        dense_ineq=False, sparse_ineq=False, use_lower=True, use_upper=True
    )
...
def optimize_truss(
    ...
    for k in range(max_iters):
        ...
        print("Iteration %d" % (k))
        print(
            "Min/max d:     %15.5e %15.5e  Total: %15.5e"
...
def optimize_truss_full(
    ...
    if start_strategy == "convex":
        ...
        # Set the output file to use
        fname = os.path.join(prefix, "truss_%s_iter%d" % (optimizer, iteration))
        opt.setOutputFile(fname)
        opt.optimize()
        iteration += 1
...
parser.add_argument(
    "--root_dir", type=str, default="results", help="Root directory to store results"
)
parser.add_argument("--optimizer", type=str, default="paropt", help="Optimizer to use")
parser.add_argument(
    "--start_strategy", type=str, default="point", help="start up strategy to use"
)
...
parser.add_argument(
    "--parameter", type=float, default=3.0, help="Penalization parameter"
)
parser.add_argument(
    "--penalization", type=str, default="SIMP", help="Penalization type"
)

...
class TrussAnalysis(ParOpt.Problem):
    def __init__(
        ...
        nwcon = self.nelems
        nwblock = 1
        super(TrussAnalysis, self).__init__(
            MPI.COMM_SELF, self.num_design_vars, ncon, nwcon, nwblock
        )
    ...
    def setNewInitPointPenalty(self, x):
        ...
        if self.penalization == "RAMP":
            ...
            for i in range(len(self.xconst)):
                ...
                self.xlinear[i] = (self.RAMP + 1.0) / (
                    1.0 + self.RAMP * (1.0 - x[i])
                ) ** 2
    ...
    def getDiscreteInfeas(self, x):
        ...
        for i in range(self.nelems):
            tnum = self.nblock * i
            d[i] = (
                1.0 - (x[tnum] - 1.0) ** 2 - sum(x[tnum + 1 : tnum + self.nblock] ** 2)
            )
    ...
    def getCompliance(self, x, limit=False):
        ...
        if limit:
            ...
            if self.penalization == "SIMP":
                for i in range(len(self.conn)):
                    for j in range(1, self.nblock):
                        ...
                        self.A[i] += (
                            self.Avals[j - 1] * x[i * self.nblock + j] ** self.SIMP
                        )
            else:
                for i in range(len(self.conn)):
                    for j in range(1, self.nblock):
                        # Compute the value of the area variable
                        val = x[i * self.nblock + j] / (
                            1.0 + self.RAMP * (1 - x[i * self.nblock + j])
                        )
            ...
            linalg.solve_triangular(
                self.L, self.u, lower=True, trans="N", overwrite_b=True
            )
            linalg.solve_triangular(
                self.L, self.u, lower=True, trans="T", overwrite_b=True
            )
    ...
    def setAreas(self, x, lb_factor=0.0):
        ...
        if self.opt_type == "convex":
            ...
            for i in range(len(self.conn)):
                for j in range(1, self.nblock):
                    # Compute the value of the area variable
                    val = self.xconst[i * self.nblock + j] + self.xlinear[
                        i * self.nblock + j
                    ] * (x[i * self.nblock + j] - self.xinit[i * self.nblock + j])
                    self.A[i] += self.Avals[j - 1] * val
        elif self.penalization == "SIMP":
            for i in range(len(self.conn)):
                for j in range(1, self.nblock):
                    # Compute the value of the area variable
                    self.A[i] += self.Avals[j - 1] * (
                        x[i * self.nblock + j] ** self.SIMP
                    )
        elif self.penalization == "RAMP":
            for i in range(len(self.conn)):
                for j in range(1, self.nblock):
                    # Compute the value of the area variable
                    val = x[i * self.nblock + j] / (
                        1.0 + self.RAMP * (1.0 - x[i * self.nblock + j])
                    )
                    self.A[i] += self.Avals[j - 1] * val
        return
    ...
    def evalObjCon(self, x):
        ...
        # Solve the resulting linear system of equations
        linalg.solve_triangular(self.L, self.u, lower=True, trans="N", overwrite_b=True)
        linalg.solve_triangular(self.L, self.u, lower=True, trans="T", overwrite_b=True)
        ...
        if self.opt_type == "convex":
            ...
            for i in range(len(self.conn)):
                ...
                gobj[i * self.nblock + 1 : (i + 1) * self.nblock] += g * (
                    self.Avals
                    * self.xlinear[i * self.nblock + 1 : (i + 1) * self.nblock]
                )
        else:
            # Add up the contribution to the gradient
            for i in range(len(self.conn)):
                ...
                if self.penalization == "SIMP":
                    # SIMP penalization
                    simp = self.SIMP * x[
                        i * self.nblock + 1 : (i + 1) * self.nblock
                    ] ** (self.SIMP - 1.0)
                    gobj[i * self.nblock + 1 : (i + 1) * self.nblock] += (
                        g * self.Avals * simp
                    )
                else:
                    # RAMP penalization
    ...
    def evalHvecProduct(self, x, z, zw, px, hvec):
        ...
        # Solve the resulting linear system of equations
        linalg.solve_triangular(
            self.L, self.phi, lower=True, trans="N", overwrite_b=True
        )
        linalg.solve_triangular(
            self.L, self.phi, lower=True, trans="T", overwrite_b=True
        )
        ...
        for i in range(len(self.conn)):
            ...
            # Find the element variables
            ue = np.array(
                [self.u[2 * n1], self.u[2 * n1 + 1], self.u[2 * n2], self.u[2 * n2 + 1]]
            )
            phie = np.array(
                [
                    self.phi[2 * n1],
    ...
    def evalSparseCon(self, x, con):
        ...
        con[:] = 2.0 * x[: n : self.nblock] - np.sum(
            x[:n].reshape(-1, self.nblock), axis=1
        )
        return
    ...
    def printTruss(self, x, filename="file.tex", draw_list=[]):
        ...
        for i in draw_list:
            ...
            xj = x[self.nblock * i + 1 + j]
            if xj > self.epsilon:
                s += "\\draw[line width=%f, color=Red]" % (2.0 * self.Avals[j] / Amin)
                s += "(%f,%f) -- (%f,%f);\n" % (
                    self.xpos[2 * n1],
                    self.xpos[2 * n1 + 1],
                    self.xpos[2 * n2],

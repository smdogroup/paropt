...
class ODESystem1(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input('x', val=np.zeros(nn),
                       desc='Horizontal Position', units='m')
        self.add_input('y', val=np.zeros(nn),
                       desc='Vertical Position', units='m')
        # self.add_input('t', val=np.zeros(nn),
        #               desc='Time', units='s')
        self.add_input('theta', val=np.zeros(nn),
                       desc='angle of wire', units='rad')
        # ###############
        # outputs
        # ###############
        self.add_output('xdot', val=np.zeros(nn),
                        desc='horizontal velocity', units='m/s')
        self.add_output('ydot', val=np.zeros(nn),
                        desc='vertical velocity', units='m/s')
        self.add_output('cflag', val=np.zeros(nn),
                        desc='collision detection', units='m') #for plotting
    ...
    def compute_partials(self, inputs, jacobian):
        ...
        #
        cflag__r0 = cflag / (1 + np.exp(-p * (r0 - 1))) \
                    * (p) * np.exp(-p * (r0 - 1))
        cflag__r1 = cflag / (1 + np.exp(-p * (r1 - 1))) \
                    * (p) * np.exp(-p * (r1 - 1))
        cflag__r2 = cflag / (1 + np.exp(-p * (r2 - 1))) \
                    * (p) * np.exp(-p * (r2 - 1))
        r0__x = (x-x0)/r0
        r0__y = (y-y0)/r0
        r1__x = (x-x1)/r1
        ...
        r2__y = (y-y2)/r2
        #
        jacobian['cflag', 'x'] = cflag__r0 * r0__x + \
                               + cflag__r1 * r1__x + \
                               + cflag__r2 * r2__x
        #
        jacobian['cflag', 'y'] = cflag__r0 * r0__y + \
                               + cflag__r1 * r1__y + \
                               + cflag__r2 * r2__y

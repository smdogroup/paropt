...
class LaunchVehicle2DEOM(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        # Inputs
        self.add_input("vx", val=np.zeros(nn), desc="x velocity", units="m/s")

        self.add_input("vy", val=np.zeros(nn), desc="y velocity", units="m/s")

        self.add_input("m", val=np.zeros(nn), desc="mass", units="kg")

        self.add_input("theta", val=np.zeros(nn), desc="pitch angle", units="rad")

        self.add_input("thrust", val=2100000 * np.ones(nn), desc="thrust", units="N")

        self.add_input(
            "Isp", val=265.2 * np.ones(nn), desc="specific impulse", units="s"
        )

        # Outputs
        self.add_output(
            "xdot", val=np.zeros(nn), desc="velocity component in x", units="m/s"
        )

        self.add_output(
            "ydot", val=np.zeros(nn), desc="velocity component in y", units="m/s"
        )

        self.add_output(
            "vxdot", val=np.zeros(nn), desc="x acceleration magnitude", units="m/s**2"
        )

        self.add_output(
            "vydot", val=np.zeros(nn), desc="y acceleration magnitude", units="m/s**2"
        )

        self.add_output(
            "mdot", val=np.zeros(nn), desc="mass rate of change", units="kg/s"
        )
    ...
    def setup(self):
        ...
        self.add_subsystem(
            ...
            om.ExecComp(
                ...
                theta={"val": np.ones(nn), "units": "rad"},
...
phase.add_parameter(
    "thrust", units="N", opt=False, val=3.0 * 50000.0 * 1.61544, targets=["eom.thrust"]
)
...
axes[0].plot(
    p.get_val("phase0.timeseries.states:x"),
...
axes[0].plot(
    exp_out.get_val("sim_traj.phase0.timeseries.states:x"),
...
axes[1].plot(
    exp_out.get_val("sim_traj.phase0.timeseries.time"),

...
/*
This class is only used when there is a single constraint and objective is linear.
...
class ParOptScaledQuasiNewton : public ParOptCompactQuasiNewton {
 public:
  ParOptScaledQuasiNewton( ParOptProblem *_prob, ParOptCompactQuasiNewton *_qn){
    ...
    if (rank == 0){
      fprintf(stdout, "[ParOptScaledQuasiNewton.h] initializing ParOptScaledQuasiNewton!\n");
    }
    qn = _qn;
    qn->incref();
  ...
  *_d = d0;
  ...
  int getMaxLimitedMemorySize(){
    return qn->getMaxLimitedMemorySize();
  }

...
void ParOptInteriorPoint::initMehrotraMultipliers(){
  ...
  // Compute the update to the slack primal variables
  if (dense_inequality){
    for ( int i = 0; i < ncon; i++ ){
      pt[i] = ps[i];
    }
  ...
  if (dense_inequality){
    ...
    for ( int i = 0; i < ncon; i++ ){
      rc[i] = penalty_gamma[i] + Ac[i]->dot(g);
    }
  }
  else {
  ...
  computeStep(ncon, z, alpha_z, pz, NULL, &zero, NULL, NULL);
  if (dense_inequality){

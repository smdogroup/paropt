...
void ParOptCompactEigenApprox::multAdd(ParOptScalar alpha, ParOptVec *x,
                                       ParOptVec *y) {
  x->mdot(hvecs, N, tmp);
...
int ParOptEigenQuasiNewton::update(ParOptVec *x, const ParOptScalar *z,
                                   ParOptVec *zw, ParOptVec *s, ParOptVec *y) {
  return 0;
}
...
void ParOptEigenQuasiNewton::multAdd(ParOptScalar alpha, ParOptVec *x,
                                     ParOptVec *y) {
  if (qn && use_quasi_newton_objective) {
...
int ParOptEigenQuasiNewton::getMaxLimitedMemorySize() { return max_vecs; }
...
int ParOptEigenQuasiNewton::getMultiplierIndex() { return index; }
...
void ParOptEigenSubproblem::setEigenModelUpdate(
    void *_data,
    void (*update)(void *, ParOptVec *, ParOptCompactEigenApprox *)) {
  if (_data && update) {
    data = _data;
...
int ParOptEigenSubproblem::evalTrialStepAndUpdate(
...
int ParOptEigenSubproblem::acceptTrialStep(ParOptVec *step, ParOptScalar *z,
                                           ParOptVec *zw) {
  int fail = 0;
...
int ParOptEigenSubproblem::getQuasiNewtonUpdateType() { return qn_update_type; }
...
*/
MPI_Comm ParOptEigenSubproblem::getMPIComm() { return prob->getMPIComm(); }
...
int ParOptEigenSubproblem::useLowerBounds() { return 1; }

int ParOptEigenSubproblem::useUpperBounds() { return 1; }

// Get the variables and bounds from the problem
void ParOptEigenSubproblem::getVarsAndBounds(ParOptVec *step, ParOptVec *l,
                                             ParOptVec *u) {
  step->zeroEntries();
  step->axpy(0.5, lk);
  step->axpy(0.5, uk);
...
*/
int ParOptEigenSubproblem::evalObjCon(ParOptVec *step, ParOptScalar *fobj,
                                      ParOptScalar *cons) {
  if (step) {
...
*/
int ParOptEigenSubproblem::evalObjConGradient(ParOptVec *step, ParOptVec *g,
                                              ParOptVec **Ac) {
...
*/
void ParOptEigenSubproblem::evalSparseCon(ParOptVec *step, ParOptVec *out) {
...
*/
void ParOptEigenSubproblem::addSparseJacobian(ParOptScalar alpha, ParOptVec *x,
...
*/
int ParOptEigenSubproblem::getLinearModel(ParOptVec **_xk, ParOptScalar *_fk,
                                          ParOptVec **_gk,
